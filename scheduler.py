...
class Scheduler:
    ...
    def sendAppointmentReminders():
        ...
        print("Reminder job started", flush=True)
        ...
        print(f"Reminder rows found: {len(rows)}", flush=True)
        ...
        for row in rows:
            ...
            print(f"Generating AI reminder for {row['first_name']}", flush=True)
            ...
            try:
                ...
                print("AI reminder generated", flush=True)
            except Exception as e:
                print("AI REMINDER ERROR:", repr(e), flush=True)
                body = None
    ...
    This is a friendly reminder that you have an upcoming appointment for {row["service_name"]} on {row["date"]}. Please reach out if you need to reschedule.
    
    Thank you,
    Maryland Contracting Company
    """
                print("Using fallback reminder body", flush=True)
    
            print(f"Sending reminder email to {recipient}", flush=True)
            ...
            print(f"Reminder email sent result: {sent}", flush=True)
        ...
        print("Reminder job finished", flush=True)
        return results
    ...
    def sendEmail(toEmail, subject, body):
        ...
        if not toEmail or not subject or not body:
            print("Missing email fields", flush=True)
            return False
        ...
        try:
            ...
            print("SendGrid status:", response.status_code, flush=True)
    
            return response.status_code == 202
    
        except Exception as e:
            print("SendGrid email error:", repr(e), flush=True)
            return False
