...
from database import init_db, seed_customers, seed_employees, seed_services, seed_appointments, seed_invoices, has_data, get_connection
...
# Flag an appointment as cancelled or completed and generate invoice if appropriate
...
def update_appointment_status():
    ...
    additional_expenses = data.get("additionalExpenses", 0)
    ...
    try:
        ...
        if additional_expenses < 0:
            return jsonify({
                "success": False,
                "message": "Additional expenses cannot be negative."
            })
    
        if additional_expenses > 10000:
            return jsonify({
                "success": False,
                "message": "Additional expenses cannot exceed $10,000."
            })
    except (ValueError, TypeError):
        return jsonify({"success": False, "message": "Invalid additional expenses value."})
