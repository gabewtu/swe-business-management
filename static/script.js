...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Loads customer first name for dashboard welcome message
    const customerWelcomeMessage = document.getElementById("customerWelcomeMessage");

    if (customerWelcomeMessage) {
        fetch("/get-customer-name")
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    customerWelcomeMessage.textContent = "Hi there, " + data.firstName + "!";
                }
            })
            .catch(() => {});
    }
    ...
    // Handles phone number data validation
    function isValidPhone(phoneNumber) {
        return /^[0-9]{10}$/.test(phoneNumber)
    }

    // Handles first & last name data validation
    ...
    if (createCustomerForm) {
        createCustomerForm.addEventListener("submit", async function (event) {
            ...
            if (!isValidPhone(phoneNumber)) {
                ...
                createCustomerMessage.textContent= "Please enter a valid phone number.";
                return;
            }
    ...
    // - Stores update data temporarily
    // - Shows confirmation modal
    const updateCustomerModal = document.getElementById("updateCustomerModal");
    const confirmUpdateCustomerButton = document.getElementById("confirmUpdateCustomerButton");
    const cancelUpdateCustomerButton = document.getElementById("cancelUpdateCustomerButton");

    let pendingUpdateCustomerData = null;
    ...
    // Confirms and submits customer update
    if (confirmUpdateCustomerButton) {
        confirmUpdateCustomerButton.addEventListener("click", async function () {
            const updateMessage = document.getElementById("updateCustomerMessage");

            if (!pendingUpdateCustomerData) {
            ...
            try {
                const response = await fetch("/update-customer", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(pendingUpdateCustomerData)
                });
            ...
            pendingUpdateCustomerData = null;
            updateCustomerModal.style.display = "none";
        });
    }

    // Cancels customer update
    if (cancelUpdateCustomerButton) {
        cancelUpdateCustomerButton.addEventListener("click", function () {
            pendingUpdateCustomerData = null;
            updateCustomerModal.style.display = "none";
        });
    }
    ...
    if (createEmployeeForm) {
        createEmployeeForm.addEventListener("submit", async function (event) {
            ...
            if (!isValidName(firstName) || !isValidName(lastName)) {
                createEmployeeMessage.style.color = "#cc0000";
                createEmployeeMessage.textContent= "Please enter a valid first and last name.";
                return;
            }
...
// Loads invoices for the currently logged-in customer
// - Calls backend route that filters by session customerID
// - Dynamically builds and displays a table of results
...
async function loadMyInvoices() {
    ...
    try {
        const response = await fetch("/get-my-invoices");
        const data = await response.json();

        if (!data.success) {
            container.innerHTML = `<p style="color:#cc0000;">${data.message || "Unable to load invoices."}</p>`;
            return;
        }

        if (data.invoices.length === 0) {
            container.innerHTML = "<p>You do not have any invoices yet.</p>";
            return;
        }
        ...
        let html = `
            <table style="width:100%; border-collapse:collapse; margin-top:10px;">
                <thead>
                    <tr>
                        <th style="text-align:left; padding:8px; border-bottom:2px solid #ccc;">Invoice #</th>
                        <th style="text-align:left; padding:8px; border-bottom:2px solid #ccc;">Invoice Date</th>
                        <th style="text-align:left; padding:8px; border-bottom:2px solid #ccc;">Appointment Date</th>
                        <th style="text-align:left; padding:8px; border-bottom:2px solid #ccc;">Service</th>
                        <th style="text-align:left; padding:8px; border-bottom:2px solid #ccc;">Employee</th>
                        <th style="text-align:right; padding:8px; border-bottom:2px solid #ccc;">Expenses</th>
        ...
        for (const invoice of data.invoices) {
            const total = invoice.service_cost + invoice.additional_expenses;

            html += `
                <tr>
                    <td>
                        <a href="/invoice/${invoice.Invoice_ID}" target="_blank">${invoice.Invoice_ID}</a>
                    </td>
                    ...
                    <td style="text-align:right;">
                        $${invoice.additional_expenses.toFixed(2)}
                    </td>
                    <td style="text-align:right;">
                        $${total.toFixed(2)}
                    </td>
        ...
        container.innerHTML = "<p style='color:#cc0000;'>An error occurred while loading invoices.</p>";
    }
}

// Loads appointments for the currently logged-in customer
// - Calls backend route that filters by session customerID
// - Builds a table of the customer's appointments
// - Handles empty and error states
async function loadMyAppointments() {
    const container = document.getElementById("myAppointmentsTableContainer");
    if (!container) return;

    container.innerHTML = "<p>Loading...</p>";

    try {
        const response = await fetch("/get-my-appointments");
        const data = await response.json();
...
// Loads and displays all invoices for the employee dashboard
async function loadAllInvoices() {
    ...
    try {
        ...
        // Catch bad HTTP responses (like 500 errors)
        if (!response.ok) {
            console.error("HTTP error:", response.status);
            container.innerHTML = "<p style='color:#cc0000;'>Failed to fetch invoices.</p>";
            return;
        }
            ...
            <table style="width:100%; border-collapse:collapse; margin-top:10px;">
                <thead>
                    <tr>
                        ...
                        <th style="text-align:left; padding:8px; border-bottom:2px solid #ccc;">Customer</th>
                        <th style="text-align:left; padding:8px; border-bottom:2px solid #ccc;">Service</th>
                        <th style="text-align:right; padding:8px; border-bottom:2px solid #ccc;">Amount</th>
                        <th style="text-align:right; padding:8px; border-bottom:2px solid #ccc;">Expenses</th>
                    </tr>
                </thead>
                <tbody>
        ...
        for (const invoice of data.invoices) {
            ...
            const cost = Number(invoice.service_cost || 0);
            const expenses = Number(invoice.additional_expenses || 0);

            html += `
                <tr>
                    <td style="padding:8px; border-bottom:1px solid #eee;">
                        <a href="/invoice/${invoice.Invoice_ID}" target="_blank">${invoice.Invoice_ID || "N/A"}</a>
                    </td>
                    <td style="padding:8px; border-bottom:1px solid #eee;">
                        ${formatDate(invoice.invoice_date)}
                    </td>
...
// Loads last 30-day financial report
// - Calls backend accounting route
// - Displays revenue, expenses, and income
async function loadReport() {
    ...
    try {
        const response = await fetch("/get-master-report");
        const data = await response.json();

        if (!data.success) {
            reportContainer.innerHTML = `
                <p style="color:#cc0000;">
                    ${data.message || "Unable to load report."}
                </p>
            `;
            return;
        }
        ...
        reportContainer.innerHTML = `
            <div style="margin-top: 15px;">
                ...
                <p><strong>Date Range:</strong> ${formatDate(report.startDate)} to ${formatDate(report.endDate)}</p>

                <table style="width:100%; border-collapse:collapse; margin-top:10px;">
                    <tr>
                        <th style="text-align:left; padding:8px; border-bottom:2px solid #ccc;">Category</th>
                        <th style="text-align:right; padding:8px; border-bottom:2px solid #ccc;">Amount</th>
                    </tr>
                    <tr>
                        <td style="padding:8px; border-bottom:1px solid #eee;">Revenue</td>
                        <td style="padding:8px; border-bottom:1px solid #eee; text-align:right;">$${report.revenue.toFixed(2)}</td>
                    </tr>
                    <tr>
                        <td style="padding:8px; border-bottom:1px solid #eee;">Expenses</td>
                        <td style="padding:8px; border-bottom:1px solid #eee; text-align:right;">$${report.expenses.toFixed(2)}</td>
                    </tr>
                    <tr>
                        <td style="padding:8px; border-bottom:1px solid #eee; font-weight:bold;">Income</td>
                        <td style="padding:8px; border-bottom:1px solid #eee; text-align:right; font-weight:bold;">$${report.income.toFixed(2)}</td>
                    </tr>
                </table>
            </div>
        `;
...
// Shows a customer dashboard section and optionally runs a loader function
function loadAndShowCustomer(sectionId, loaderFn) {
    showDashboardSection(sectionId);
    if (loaderFn) loaderFn();
}

// Opens/closes sidebar submenu groups
function toggleGroup(groupId, btn) {
    ...
    document.querySelectorAll(".sub-menu").forEach(g => g.classList.remove("open"));
    document.querySelectorAll(".group-button").forEach(b => b.classList.remove("open"));
...
// Loads services into the employee dashboard services table
    ...
    fetch("/get-all-services")
        ...
        .then(data => {
            ...
            for (const svc of data.services) {
                html += `
                    <tr>
                        <td style="padding:8px; border-bottom:1px solid #eee; font-weight:bold;">${svc.name}</td>
                        <td style="padding:8px; border-bottom:1px solid #eee; color:#555;">${svc.description || "—"}</td>
                        <td style="padding:8px; border-bottom:1px solid #eee; text-align:right;">$${svc.cost.toFixed(2)}</td>
                    </tr>
                `;
            }
        ...
        .catch(() => {
            container.innerHTML = "<p style='color:#cc0000;'>Error loading services.</p>";
        });
}